...
const HalfAndHalfPizza = ({ handleCloseHalfAndHalfPizza  }) => {
  ...
  const FIXED_HALF_AND_HALF_COST = 2.0;
  ...
  useEffect(() => {
    if (leftSelectedPizza && rightSelectedPizza) {
      let calculatedPrice = 0;

      // Calculate price for the left half
      let leftPrice = 0;
      switch (leftSize) {
        case 'Large':
          leftPrice = leftSelectedPizza.LargePrice / 2;
          break;
        case 'Extra Large':
          leftPrice = leftSelectedPizza.ExtraLargePrice / 2;
          break;
        default:
          leftPrice = 0;
      }

      let leftAdditionalCost = 0;
      if (leftCrust) leftAdditionalCost += FIXED_INGREDIENT_COST;
      if (leftCheese) leftAdditionalCost += FIXED_INGREDIENT_COST;
      if (leftSauce) leftAdditionalCost += FIXED_INGREDIENT_COST;
      leftAdditionalCost += leftSelectedIngredients.length * FIXED_INGREDIENT_COST;
      leftPrice += leftAdditionalCost;

      // Calculate price for the right half
      let rightPrice = 0;
      switch (rightSize) {
        case 'Large':
          rightPrice = rightSelectedPizza.LargePrice / 2;
          break;
        case 'Extra Large':
          rightPrice = rightSelectedPizza.ExtraLargePrice / 2;
          break;
        default:
          rightPrice = 0;
      }

      let rightAdditionalCost = 0;
      if (rightCrust) rightAdditionalCost += FIXED_INGREDIENT_COST;
      if (rightCheese) rightAdditionalCost += FIXED_INGREDIENT_COST;
      if (rightSauce) rightAdditionalCost += FIXED_INGREDIENT_COST;
      rightAdditionalCost += rightSelectedIngredients.length * FIXED_INGREDIENT_COST;
      rightPrice += rightAdditionalCost;

      calculatedPrice = leftPrice + rightPrice / 2; // Combine both prices without dividing by 2

      setPrice(calculatedPrice);
    }
}, [leftSelectedPizza, rightSelectedPizza, leftSize, rightSize, leftCrust, leftCheese, leftSauce, leftSelectedIngredients, rightCrust, rightCheese, rightSauce, rightSelectedIngredients, quantity, FIXED_INGREDIENT_COST]);
